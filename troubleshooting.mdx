...
description: "Common issues and solutions for preview-deployer"
---
...
**Cause**: The orchestrator requires a valid `preview-config.yml` at the repository root. Required fields: `framework`, `database`, `health_check_path`, `app_port`, `app_port_env`, `app_entrypoint`.

**Solutions**:

1. Add `preview-config.yml` in the repo root (see [Configuration Reference](/configuration)).
...
3. Cleanup old deployment (use the full deployment id from `GET /api/previews`, e.g. `my-org-my-app-12`):
   ```bash
   curl -X DELETE http://SERVER_IP:3000/api/previews/{deploymentId}
   ```
   See [Orchestrator API](/reference/orchestrator-api) for full endpoint details.
...
4. Check deployment age:
   ```bash
   cat /opt/preview-deployer/deployments.json | jq '.deployments."{deploymentId}"'
   ```
...
Test orchestrator API (see [Orchestrator API](/reference/orchestrator-api) for full endpoint reference):
...
3. Check [Configuration Reference](/configuration)
